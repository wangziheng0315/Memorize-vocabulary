...
本期要解决的状态是“某个用户在某本书学到哪里了”以及已成功学习的单词明细。`user_book_progress` 保存书级断点和汇总，`user_word_progress` 保存当前学习周期的单词记录；不引入学习流水或复习计划。
...
2. 变更数据使用 Next.js Server Actions；首页最近学习通过受保护的 `GET /api/progress/recent` 获取，数据库访问始终留在服务端。
...
### 4.4 单词级进度：`public.user_word_progress`

每行代表当前学习周期内用户成功推进过的一个单词。`UNIQUE (user_id, book_id, word_id)` 保证重复提交幂等；完成后重新开始时，事务会清理该用户该书的旧周期记录，再写入第一词。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `bigint identity` | 技术主键 |
| `user_id`、`book_id`、`word_id` | `integer`、`text`、`bigint` | 用户、单词书和 `words.id` 外键 |
| `learned_at` | `timestamptz` | 最近一次成功推进时间 |
| `created_at`、`updated_at` | `timestamptz` | 行审计时间 |

该表只接受服务端校验过的“当前书内单词”，不信任客户端传入的书籍归属或进度数量。
...
create table public.user_word_progress (
  ...
  constraint user_word_progress_user_fkey foreign key (user_id) references public."User"(id),
  constraint user_word_progress_book_fkey foreign key (book_id) references public.books(book_id),
  constraint user_word_progress_word_fkey foreign key (word_id) references public.words(id)
);
...
| `GET /api/progress/recent` | 首页登录态 | 服务端从 session 推导用户，联结单词书并按 `last_studied_at DESC` 返回 3 本 |
...
首页登录后由 `RecentLearning` 客户端组件请求 `GET /api/progress/recent`。未登录或无进度时不渲染模块；接口固定按当前 session 用户过滤并最多返回 3 条，禁止接受 `userId` 查询参数。
...
单词导入/更新完成后必须执行 4.6 的数量校验。若 `books.word_count` 与实际不一致，列表显示和完成百分比可能不同；上线前应修正 `word_count`，而不是在前端猜测。
...
1. 在预发布库执行数据检查，建立认证用户约束、`words → books` 关系、索引以及两张用户进度表。
...
5. 实现学习页、详情页、JSON DTO 适配、最近学习 API 和同步两张进度表的 `advanceStudy` 事务。
6. 按下列测试清单验证，再切换正式流量。
